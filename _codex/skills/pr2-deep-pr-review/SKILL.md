...
metadata:
  short-description: "8-pass PR review with parallel finding validation"
---

# PR2 - Deep Pull Request Review

Use this workflow when user asks for a deep PR review, or when pre-commit self-review needs strict validation.

## Inputs
- PR reference argument: number or URL
- If no argument: review current branch PR with `gh pr view`
...
- Glob
- Grep
- Task (or equivalent sub-agent tool)

## Phase 1: Fetch PR Context
1. Load PR metadata:
   - `gh pr view <arg>`
2. Collect changed file list:
...
5. Capture:
   - title, description, stated problem/goal
   - author, base, head
   - linked issues in body
6. Read every changed file in full before review passes.
...
Print this exact structure before validation:
...
Do not pre-filter findings before validation.

## Phase 4: Parallel Sub-Agent Validation
For every candidate finding, run one independent sub-agent in parallel.

Sub-agent input must include:
1. Full PR diff
2. Finding details (`id`, `category`, `title`, `location`, `evidence`, `hypothesis`)
3. Validation instruction:
   - verdict: `valid | invalid | partial`
   - impact: `critical | high | medium | low | informational`
   - 2-4 sentence evidence-based justification
   - concrete fix if valid
